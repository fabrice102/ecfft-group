...
pub fn read_coset<F: PrimeField, const NUM_LIMBS: usize, ToF>(file_name: &str, vec_to_field_element: ToF) -> Vec<F>
    where
        ToF: Fn(&[u64]) -> F {
    std::fs::read_to_string(file_name)
        .unwrap_or_else(|_| panic!("Run `get_params.sage` to generate the coset file {}.", file_name))
        .split_whitespace()
...
/// Uses the closure `vec_to_field_element` to convert a vector of `NUM_LIMBS` `u64`
/// into a field element.
pub fn read_isogenies<F: PrimeField, const NUM_LIMBS: usize, ToF>(file_name: &str, to_field_element: ToF) -> Vec<Isogeny<F>>
    where
        ToF: Fn(&[u64]) -> F {
    std::fs::read_to_string(file_name)
        .unwrap_or_else(|_| panic!("Run `get_params.sage` to generate the coset file {}.", file_name))
